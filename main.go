...
func (r *Room) BroadcastRTP(pkt *rtp.Packet, senderID string) {
	...
	for _, session := range r.sessions {
		...
		if err := session.audioTrack.WriteRTP(pkt); err != nil {
			slog.Error(
				"write RTP",
		...
		if track.Kind() == webrtc.RTPCodecTypeAudio {
			go func() {
				for {
					...
					if err != nil {
						slog.Error("RTP read error", constant.Error, err)
...
func (h *HttpHandler) handleMessage(session *Session, msg *signaling.Message) error {
	switch msg.Type {
	case "join":
